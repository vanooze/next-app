...
interface UploadMarketingReportProps {
  isOpen: boolean;
  onClose: () => void;
  taskId: number;
  folder?: string; // optional folder name, default "marketing"
}

export const UploadMarketingReport: React.FC<UploadMarketingReportProps> = ({
  isOpen,
  onClose,
  taskId,
  folder = "marketing",
  ...
  const handleUploadFile = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/department/MARKETING/tasks/upload", {
        method: "POST",
        body: formData,
      });
      ...
      // Refresh task files
      await mutate(`/api/department/MARKETING/tasks/files?taskId=${taskId}`);
