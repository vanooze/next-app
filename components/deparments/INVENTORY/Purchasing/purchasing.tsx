...
export const Purchasing = () => {
  ...
  const {
    ...
    error,
    isLoading,
  } = useSWR<POMonitoring[]>("/api/department/INVENTORY/purchasing", fetcher, {
  ...
  const filteredTasks = debouncedFilterValue
    ? tasks.filter((task) => {
        ...
        return (
          ...
          task.supplier?.toLowerCase().includes(query) ||
          task.items.toLowerCase().includes(query)
        );
      })
    : tasks;
  ...
  return (
    <div className="my-10 px-4 lg:px-6 max-w-[95rem] mx-auto w-full flex flex-col gap-4">
      <ul className="flex">
        ...
        <li className="flex gap-2">
          <span>PO Monitoring</span>
        </li>
      </ul>

      <h3 className="text-xl font-semibold">PO Monitoring</h3>
