...
export default function Proposal({ project }: ProposalProps) {
    ...
    const fetchassignedPersonnel = async () => {
      ...
      try {
        const res = await fetch(
          `/api/department/PMO/project_tasks/so/proposal/personnel/get?projectId=${projectId}`
        );
        const data = await res.json();
  ...
  const submitForm = async () => {
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/so/proposal/create",
          {
            method: "POST",
            body: formData,
          }
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      <h1 className="text-lg font-semibold">Assign Proposal to:</h1>
          ...
          onSelectionChange={async (keys) => {
            ...
            if (typeof selected === "string") {
              ...
              if (projectId) {
                try {
                  await fetch(
                    "/api/department/PMO/project_tasks/so/proposal/personnel",
                    {
                      method: "POST",
                  ...
                  console.error("Failed to update assigned Proposal:", err);
                }
              }
            }
          }}
      ...
      {assignedPersonnel && canUpload && (
        <>
          ...
          {/* PO Attachment */}
          <h1 className="text-lg font-semibold">Proposal Attachment</h1>
