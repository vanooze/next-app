...
import { VALUE_ENG_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function ValueEng({ project }: ValueEngProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/postProject/value_eng?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/postProject/value_eng/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    VALUE_ENG_CAN_UPLOAD_DESIGNATION.some((role) =>
