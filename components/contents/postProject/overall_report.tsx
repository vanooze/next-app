...
import { OVERALL_REPORT_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function OverallReport({ project }: OverallReportProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/postProject/overallReport?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/postProject/overallReport",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    OVERALL_REPORT_CAN_UPLOAD_DESIGNATION.some((role) =>
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      {canUpload && (
        <>
          <h1 className="text-lg font-semibold">Overall Report Details</h1>
          <Textarea
            className="max-w-lg"
            label="Overall Report Details"
            placeholder="Enter the details here..."
            value={Details}
            onChange={(e) => setDetails(e.target.value)}
          />

          <h1 className="text-lg font-semibold">Overall Report Attachment</h1>
          ...
          <a
            href="/form/Overall Report.docx"
            download
            className="text-blue-600 text-sm hover:underline mt-2 max-w-lg"
          >
            📄 Download Overall Report Format (DOCX)
          </a>
        </>
      )}
      <Divider />
