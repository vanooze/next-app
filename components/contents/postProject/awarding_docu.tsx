...
import { AWARDING_DOCU_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";

interface AwardingDocumentsProps {
...
export default function AwardingDocuments({ project }: AwardingDocumentsProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/postProject/awarding_docu?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/postProject/awarding_docu/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const hasAccess =
    user?.designation &&
    AWARDING_DOCU_CAN_UPLOAD_DESIGNATION.some((role) =>
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      {hasAccess && (
        <>
          <h1 className="text-lg font-semibold">Awarding Documents Details</h1>
          <Textarea
            className="max-w-lg"
            label="Awarding Documents Details"
