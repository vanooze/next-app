...
export default function ManPowerCount({ project }: ManPowerCountProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/postProject/manpower_count?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/postProject/manpower_count/create",
          {
            method: "POST",
            body: formData,
          }
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      {/* PO Details */}
      <h1 className="text-lg font-semibold">Final Man Power Count Details</h1>
      <Textarea
        className="max-w-lg"
        label="Final Man Power Count Details"
      ...
      <h1 className="text-lg font-semibold">
        Final Man Power Count Attachment
      </h1>
