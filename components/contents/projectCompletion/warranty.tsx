...
import { WARRANTY_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function Warranty({ project }: WarrantyProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/warranty?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/warranty/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    WARRANTY_CAN_UPLOAD_DESIGNATION.some((role) =>
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      {canUpload && (
        <>
          <h1 className="text-lg font-semibold">Warranty Details</h1>
          <Textarea
            className="max-w-lg"
          ...
          <h1 className="text-lg font-semibold">Warranty Attachment</h1>
