...
import { TRAINEE_ACCEPTANCE_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function Trainee({ project }: TraineeProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/trainee_acceptance?id=${projectId}`
    : null;
  ...
  const submitForm = async () => {
    ...
    const status = "1";
    const type = fileType; // Changed here
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/trainee_acceptance/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    TRAINEE_ACCEPTANCE_CAN_UPLOAD_DESIGNATION.some((role) =>
