...
import { DR_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function DR({ project }: DRProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/dr?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/dr/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    DR_CAN_UPLOAD_DESIGNATION.some((role) =>
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      {canUpload && (
        <>
          <h1 className="text-lg font-semibold">Delivery Receipt Details</h1>
          <Textarea
            className="max-w-lg"
            label="Delivery Receipt Details"
            placeholder="Enter the details here..."
            value={Details}
            onChange={(e) => setDetails(e.target.value)}
          />

          <h1 className="text-lg font-semibold">Delivery Receipt Attachment</h1>
