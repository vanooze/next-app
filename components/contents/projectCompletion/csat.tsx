...
export default function CSAT({ project }: CSATProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/csat?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/csat/create",
          {
            method: "POST",
            body: formData,
          }
    ...
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      {canUpload && (
        <>
          ...
          <Textarea
            className="max-w-lg"
            label="Customer Satisfaction Survey Details"
