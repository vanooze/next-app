...
import { COC_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function COC({ project }: COCProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/coc?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/coc/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    COC_CAN_UPLOAD_DESIGNATION.some((role) =>
    ...
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      {canUpload && (
        <>
          ...
          <Textarea
            className="max-w-lg"
            label="Certificate of Completion Details"
