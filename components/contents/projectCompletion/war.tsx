...
import { WAR_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function WAR({ project }: WARProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/war?id=${projectId}`
    : null;
  ...
  const submitForm = async () => {
    ...
    const status = "1";
    const type = "COMPLETION"; // Changed here
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/war/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    WAR_CAN_UPLOAD_DESIGNATION.some((role) =>
      ...
      {canUpload && (
        <>
          <h1 className="text-lg font-semibold">
            Work Activity Report Details
          </h1>
          <Textarea
            className="max-w-lg"
            label="Work Activity Report Details"
            placeholder="Enter the details here..."
            value={Details}
            onChange={(e) => setDetails(e.target.value)}
          />
          <h1 className="text-lg font-semibold">
            Work Activity Report Attachment
          </h1>
