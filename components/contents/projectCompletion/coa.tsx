...
import { COA_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function COA({ project }: COAProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/coa?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/coa/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    COA_CAN_UPLOAD_DESIGNATION.some((role) =>
      ...
      {canUpload && (
        <>
          <h1 className="text-lg font-semibold">
            Certificate of Acceptance Details
          </h1>
          <Textarea
            className="max-w-lg"
            label="Certificate of Acceptance Details"
          ...
          <h1 className="text-lg font-semibold">
            Certificate of Acceptance Attachment
          </h1>
