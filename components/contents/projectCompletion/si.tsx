...
import { SI_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function SI({ project }: SIProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/si?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/si/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    SI_CAN_UPLOAD_DESIGNATION.some((role) =>
