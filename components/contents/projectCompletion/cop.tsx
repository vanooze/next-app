...
import { COP_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function COP({ project }: COPProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectCompletion/cop?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectCompletion/cop/create",
          {
            method: "POST",
            body: formData,
          },
  ...
  const canUpload =
    user?.designation &&
    COP_CAN_UPLOAD_DESIGNATION.some((role) =>
      ...
      {canUpload && (
        <>
          <h1 className="text-lg font-semibold">
            Certificate of Good Performance Details
          </h1>
          <Textarea
            className="max-w-lg"
            label="Certificate of Good Performance Details"
          ...
          <h1 className="text-lg font-semibold">
            Certificate of Good Performance Attachment
          </h1>
