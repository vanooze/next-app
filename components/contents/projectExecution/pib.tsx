...
export default function PIB({ project }: PIBProps) {
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectexecution/pib?id=${projectId}`
    : null;
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectexecution/pib/create",
          {
            method: "POST",
            body: formData,
          }
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      {/* PO Details */}
      <h1 className="text-lg font-semibold">Daily PIB Details</h1>
      <Textarea
        className="max-w-lg"
        label="Daily PIB Details"
      ...
      {/* PO Attachment */}
      <h1 className="text-lg font-semibold">Daily PIB Attachment</h1>
