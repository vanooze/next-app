...
import { SIGNED_CONCEPTUAL_CAN_UPLOAD_DESIGNATION } from "@/helpers/restriction";
...
export default function Conceptual({ project }: ConceptualProps) {
  ...
  const canUpload =
    user?.designation &&
    SIGNED_CONCEPTUAL_CAN_UPLOAD_DESIGNATION.some((role) =>
  ...
  const submitForm = async () => {
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectkickoff/conceptual/create",
          {
            method: "POST",
            body: formData,
          },
