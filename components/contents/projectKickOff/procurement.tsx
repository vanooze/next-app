...
export default function Procurement({ project }: ProcurementProps) {
    ...
    const fetchassignedPersonnel = async () => {
      ...
      try {
        const res = await fetch(
          `/api/department/PMO/project_tasks/projectkickoff/procurement/personnel/get?projectId=${projectId}`
        );
        const data = await res.json();
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectkickoff/procurement?id=${projectId}`
    : null;
  ...
  const submitForm = async () => {
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectkickoff/procurement/create",
          {
            method: "POST",
            body: formData,
          }
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      <h1 className="text-lg font-semibold">Assign SOW to:</h1>
          ...
          onSelectionChange={async (keys) => {
            ...
            if (typeof selected === "string") {
              ...
              if (projectId) {
                try {
                  await fetch(
                    "/api/department/PMO/project_tasks/projectkickoff/procurement/personnel",
                    {
                      method: "POST",
                  ...
                  console.error("Failed to update assigned SOW:", err);
                }
              }
            }
          }}
      ...
      {assignedPersonnel && canUpload && (
        <>
          {/* PO Details */}
          <h1 className="text-lg font-semibold">SOW Details</h1>
          <Textarea
            className="max-w-lg"
            label="SOW Details"
          ...
          {/* PO Attachment */}
          <h1 className="text-lg font-semibold">SOW Attachment</h1>
