...
export default function Equipment({ project }: EquipmentProps) {
    ...
    const fetchassignedPersonnel = async () => {
      ...
      try {
        const res = await fetch(
          `/api/department/PMO/project_tasks/projectValidation/equipment/personnel/get?projectId=${projectId}`
        );
        const data = await res.json();
  ...
  const key = projectId
    ? `/api/department/PMO/project_tasks/projectValidation/equipment?id=${projectId}`
    : null;
  ...
  const submitForm = async () => {
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectValidation/equipment/create",
          {
            method: "POST",
            body: formData,
          }
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      <h1 className="text-lg font-semibold">Assign Equipment to:</h1>
          ...
          onSelectionChange={async (keys) => {
            ...
            if (typeof selected === "string") {
              ...
              if (projectId) {
                try {
                  await fetch(
                    "/api/department/PMO/project_tasks/projectValidation/equipment/personnel",
                    {
                      method: "POST",
      ...
      {assignedPersonnel && canUpload && (
        <>
          ...
          {/* PO Attachment */}
          <h1 className="text-lg font-semibold">Equipment Attachment</h1>
