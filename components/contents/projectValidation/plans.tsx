...
export default function Plans({ project }: PlansProp) {
    ...
    const fetchassignedPersonnel = async () => {
      ...
      try {
        const res = await fetch(
          `/api/department/PMO/project_tasks/projectValidation/plans/personnel/get?projectId=${projectId}`
        );
        const data = await res.json();
  ...
  const submitForm = async () => {
    ...
    try {
      for (const file of files) {
        ...
        const res = await fetch(
          "/api/department/PMO/project_tasks/projectValidation/plans/create",
          {
            method: "POST",
            body: formData,
          }
  ...
  return (
    <div className="flex w-full flex-col md:flex-nowrap gap-4">
      <h1 className="text-lg font-semibold">Assign Plans to:</h1>
          ...
          onSelectionChange={async (keys) => {
            ...
            if (typeof selected === "string") {
              ...
              if (projectId) {
                try {
                  await fetch(
                    "/api/department/PMO/project_tasks/projectValidation/plans/personnel",
                    {
                      method: "POST",
      ...
      {assignedPersonnel && canUpload && (
        <>
          {/* PO Details */}
          <h1 className="text-lg font-semibold">Plans Details</h1>
          <Textarea
            className="max-w-lg"
            label="Plans Details"
          ...
          {/* PO Attachment */}
          <h1 className="text-lg font-semibold">Plans Attachment</h1>
