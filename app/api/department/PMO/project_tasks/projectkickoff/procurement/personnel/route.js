...
export async function POST(req) {
  try {
    ...
    if (existing.length > 0) {
      await executeQuery(
        `UPDATE procurement SET assigned_personnel = ? WHERE project_id = ?`,
      ...
      const result = await executeQuery(
        `INSERT INTO procurement (project_id, project_name, assigned_personnel) VALUES (?, ?, ?)`,
    ...
    return NextResponse.json(
      { success: false, error: "Failed to save procurement" },
      { status: 500 }
    );
  }
}
