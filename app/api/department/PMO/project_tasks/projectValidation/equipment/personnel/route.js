...
export async function POST(req) {
  try {
    ...
    const existing = await executeQuery(
      `SELECT * FROM equipment WHERE project_id = ?`,
      [projectId]
    );

    if (existing.length > 0) {
      await executeQuery(
        `UPDATE equipment SET assigned_personnel = ? WHERE project_id = ?`,
      ...
      const result = await executeQuery(
        `INSERT INTO equipment (project_id, assigned_personnel) VALUES ( ?, ?)`,
