...
export async function POST(req) {
  try {
    ...
    const [existing] = await executeQuery(
      `SELECT file_name FROM it_activity WHERE id = ?`,
      [taskId],
    );
    ...
    if (existing?.file_name) {
      try {
        allFiles = JSON.parse(existing.file_name);
      } catch {
        allFiles = [];
      }
    }
    ...
    // Update table
    await executeQuery(`UPDATE it_activity SET file_name = ? WHERE id = ?`, [
...
export async function GET(req) {
  try {
    ...
    if (row && row.file_name) {
      try {
        const attachments = JSON.parse(row.file_name); // JSON array
        ...
        console.error("Failed to parse file_name JSON:", err);
      }
    }
