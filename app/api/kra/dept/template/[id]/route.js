...
export async function POST(req, { params }) {
  try {
    ...
    await executeQuery(`DELETE FROM kra_scores_table_dept WHERE user_id = ?`, [
      employeeId,
    ]);

    // Fetch the template
    const templateRows = await executeQuery(
      `SELECT * FROM kra_table_template_dept WHERE id = ?`,
      [templateId],
    );
    ...
    const insertQueries = kraRows.map((row) =>
      executeQuery(
        `INSERT INTO kra_scores_table_dept
