import {
  getDesignTaskSchema,
  GetDesignTaskArgs,
} from "../schemas/getDesignTaskSchema";
import { resolveTaskPeople, personMatches } from "../utils/taskResolver";
import { statusMatches } from "../utils/normalize";

export function registerGetDesignTaskTool(server: any) {
  server.tool(
    "get-design-task",
    getDesignTaskSchema,
    async ({ status, person, details, debug, trace }: GetDesignTaskArgs) => {
      ...
      const traced = trace
        ? statusFiltered.map((t) => ({
            ...
            resolvedPeople: resolveTaskPeople(t),
            includesPerson: personMatches(resolveTaskPeople(t), person),
          }))
        : [];
